...
| E4 | Scene saving | "Save" writes the current (possibly edited) entity state back to a `.scene.json` using the existing schema (`engine::scene`/`SceneDocument`'s JSON format) — round-trips through the same format `LoadScene` already reads, no new format invented. | ✅ Verified on Linux desktop and physical Pi4 hardware (edit → Save → file on disk changed → fresh relaunch loads the saved value, confirmed end to end on real Pi4 hardware) |
