...
| Asset Browser | Partial | Thumbnails, folders, drag-drop into Scene View/Inspector, search | Source Assets is now a real folder tree (recursed from assets/, grouped by path component the same way the Hierarchy panel groups by parent) instead of a flat list, with a thumbnail column -- a real decoded-image preview for a `.png` (this project's first ImGui-displayed GPU texture, `ImGuiOverlay::RegisterTexture()`/`ImGui_ImplVulkan_AddTexture()`) or a colored type-icon otherwise (mesh/material/shader, `ImGui::ColorButton`, deliberately not a Unicode glyph -- ImGui's default font has no block-element characters, and stb_image's own imprecise TGA-format sniffing turned out to spuriously "succeed" on arbitrary non-image files during this feature's own Pi4 testing, so decoding is now gated on the `.png` extension up front, not attempted-then-checked). Drag a file out of the tree to drop it: a mesh (`.gltf`/`.glb`) onto the Scene View spawns a new entity with it (parallels "Create Cube"), a material (`.material.json`) onto the Inspector's "Assign Material" section assigns it. Cooked Output stays a flat list (GUID-named Cooker output, never user-organized into folders) -- no search yet |
