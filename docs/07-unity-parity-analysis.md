...
| Lighting / PBR | Partial (PBR half still a non-goal) | Realtime GI, light probes, reflection probes | Lighting phase A (docs/01 section 8.3's "Low-Poly Retro" profile) is done: `LightComponent` (Directional/Point, up to `kMaxLights`=4 simultaneous, docs/01's own indicative budget), `ForwardLitShadedPipeline` (a fourth-then-fifth concrete pipeline, minimal Blinn-Phong -- ambient + N·L diffuse + a fixed-shininess specular term), a per-frame lighting UBO (this project's first GPU resource written more than once, double-buffered across frames-in-flight). The PBR half of this row is still the explicit non-goal `CLAUDE.md` section 1 names -- `ForwardPlusPBRPipeline`/metallic-roughness/IBL remain unimplemented, this is the "Low-Poly Retro" profile's own lighting model, not a scaled-down PBR one. No shadows yet either (a static-only shadow map is a separate, not-yet-started follow-up, docs/01 section 8.3's own "preferably baked" shadow guidance) |
