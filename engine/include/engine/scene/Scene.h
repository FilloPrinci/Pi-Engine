...
// Prefab::Instantiate). `createPhysicsBody`/`attachScript` are both optional, see
// SceneDocument.h's own comments for why these are callbacks rather than a
// physics::PhysicsWorld*/script::ScriptRegistry& (engine/scene/ stays physics- and
// script-agnostic on purpose). Returns false if the file couldn't be parsed -- parsing
// happens fully before anything is spawned, so a failure never leaves `world` with a
// half-loaded scene.
