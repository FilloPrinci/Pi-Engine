...
// Same depth-format fallback chain as every sample since M1 (see m1_hello_mesh's identical
...
// M7 -- Asset Pipeline LOD-generation step (docs/01 section 12.2, 5th and last of 5 Asset
// Pipeline sub-parts). Exit criterion: `cooker mesh` decimates a source mesh into multiple
// LOD levels (meshoptimizer's edge-collapse simplifier), packed into one cooked mesh file
// sharing a single vertex buffer (CookedMesh.h version 4) -- this sample loads every LOD's
// index buffer up front and lets Space cycle which one is drawn, proving the whole
// generation+storage+selection path end to end. There is no Hardware Profile System yet
// (CLAUDE.md section 8) to pick a LOD automatically by distance/profile -- that's future
// work once one exists; this milestone is just "the Cooker can produce usable LODs".
int main(int /*argc*/, char** /*argv*/) {
    std::printf("Pi-Engine %s -- m7_lod\n", engine::core::GetEngineVersionString());
    ...
    RHIContext context;
    if (!context.Init(displayBackend, "m7_lod")) {
        return EXIT_FAILURE;
    }
    ...
    // --- Mesh: every LOD level `cooker mesh` generated, loaded up front. All LODs share
    //     one vertex buffer (CookedMesh.h's format), so only one vertex upload is needed;
    //     each LOD gets its own index buffer since that's the only thing that differs. ---
    const std::string meshPath = CookedAssetPath("m7_lod_sphere.mesh");

    std::uint32_t lodCount = 0;
    if (!engine::renderer::GetCookedMeshLODCount(meshPath.c_str(), lodCount) || lodCount == 0) {
        std::fprintf(stderr, "m7_lod: failed to read LOD count from \"%s\"\n", meshPath.c_str());
        return EXIT_FAILURE;
    }

    RHIBuffer vertexBuffer;
    std::vector<std::unique_ptr<RHIBuffer>> indexBuffers(lodCount);
    std::vector<std::uint32_t> indexCounts(lodCount);

    for (std::uint32_t lod = 0; lod < lodCount; ++lod) {
        MeshData mesh;
        if (!engine::renderer::LoadCookedMesh(meshPath.c_str(), mesh, nullptr, lod)) {
            std::fprintf(stderr, "m7_lod: failed to load LOD %u from \"%s\"\n", lod,
                         meshPath.c_str());
            return EXIT_FAILURE;
        }
        std::printf("m7_lod: LOD %u -- %zu vertices, %zu triangles\n", lod, mesh.vertices.size(),
                    mesh.indices.size() / 3);

        if (lod == 0) {
            // The shared vertex buffer -- identical content for every LOD, only uploaded once.
            if (!vertexBuffer.InitWithData(context, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                                            mesh.vertices.data(),
        ...
        indexBuffers[lod] = std::make_unique<RHIBuffer>();
        if (!indexBuffers[lod]->InitWithData(context, VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                                              mesh.indices.data(),
                                              mesh.indices.size() * sizeof(mesh.indices[0]))) {
            return EXIT_FAILURE;
        }
        indexCounts[lod] = static_cast<std::uint32_t>(mesh.indices.size());
    }
    ...
    if (depthFormat == VK_FORMAT_UNDEFINED) {
        std::fprintf(stderr, "m7_lod: no supported depth/stencil format found\n");
        return EXIT_FAILURE;
    }
    ...
    auto createDepthResources = [&](VkExtent2D extent) -> bool {
        ...
        if (vmaCreateImage(context.GetAllocator(), &imageInfo, &allocInfo, &depthImage,
            ...
            std::fprintf(stderr, "m7_lod: vmaCreateImage (depth) failed\n");
            return false;
        }
        ...
        if (vkCreateImageView(device, &viewInfo, nullptr, &depthView) != VK_SUCCESS) {
            std::fprintf(stderr, "m7_lod: vkCreateImageView (depth) failed\n");
            return false;
        }
        return true;
    };
    ...
    if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &renderPass) != VK_SUCCESS) {
        std::fprintf(stderr, "m7_lod: vkCreateRenderPass failed\n");
        return EXIT_FAILURE;
    }

    // --- Pipeline -- plain unlit, no texture (this step is about geometry, reuses M1's
    //     pipeline as-is; Vertex's uv field is simply unused here). ---
    ...
    auto createFramebuffers = [&]() -> bool {
        for (std::size_t i = 0; i < framebuffers.size(); ++i) {
            ...
            if (vkCreateFramebuffer(device, &framebufferInfo, nullptr, &framebuffers[i]) !=
                VK_SUCCESS) {
                std::fprintf(stderr, "m7_lod: vkCreateFramebuffer failed for image %zu\n", i);
                return false;
            }
        }
        return true;
    ...
    if (vkCreateCommandPool(device, &poolInfo, nullptr, &commandPool) != VK_SUCCESS) {
        std::fprintf(stderr, "m7_lod: vkCreateCommandPool failed\n");
        return EXIT_FAILURE;
    }
    ...
    if (vkAllocateCommandBuffers(device, &allocInfo, commandBuffers) != VK_SUCCESS) {
        std::fprintf(stderr, "m7_lod: vkAllocateCommandBuffers failed\n");
        return EXIT_FAILURE;
    }
    ...
    for (int i = 0; i < kMaxFramesInFlight; ++i) {
        if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imageAvailable[i]) != VK_SUCCESS ||
            ...
            std::fprintf(stderr, "m7_lod: failed to create sync objects for frame %d\n", i);
            return EXIT_FAILURE;
        }
    }

    // --- Main loop ---
    std::printf("m7_lod: running -- press Space to cycle LOD, close the window to exit.\n");
    InputState rawInput;
    InputSystem input;
    Camera camera;
    int currentFrame = 0;
    std::uint32_t currentLod = 0;
    bool running = true;
    ...
    auto updateTitle = [&](double fps) {
        char title[96];
        std::snprintf(title, sizeof(title), "Pi-Engine -- m7_lod (%.0f FPS, LOD %u/%u, %u tris)",
                      fps, currentLod, lodCount - 1, indexCounts[currentLod] / 3);
        displayBackend.SetWindowTitle(title);
    };

    while (running) {
        displayBackend.PollEvents(rawInput);
        if (rawInput.quitRequested) {
            break;
        }
        input.Update(rawInput);
        if (input.WasPressedThisFrame(Key::Space)) {
            currentLod = (currentLod + 1) % lodCount;
            std::printf("m7_lod: switched to LOD %u (%u triangles)\n", currentLod,
                        indexCounts[currentLod] / 3);
            updateTitle(lastReportedFps);
        }
        ...
        // Same auto-orbit stopgap as every sample without a free-look camera (M1/M2/M7
        // textures/quad) -- proves the view/projection math and gives every LOD's silhouette
        // a chance to be seen from multiple angles.
        ...
        if (acquireResult != VK_SUCCESS && acquireResult != VK_SUBOPTIMAL_KHR) {
            std::fprintf(stderr, "m7_lod: vkAcquireNextImageKHR failed\n");
            break;
        }
        ...
        vkCmdBindIndexBuffer(cmd, indexBuffers[currentLod]->GetHandle(), 0, VK_INDEX_TYPE_UINT32);
        vkCmdDrawIndexed(cmd, indexCounts[currentLod], 1, 0, 0, 0);
        ...
        if (vkQueueSubmit(context.GetGraphicsQueue(), 1, &submitInfo, inFlight[currentFrame]) !=
            VK_SUCCESS) {
            std::fprintf(stderr, "m7_lod: vkQueueSubmit failed\n");
            break;
        }
        ...
        if (presentResult == VK_ERROR_OUT_OF_DATE_KHR || presentResult == VK_SUBOPTIMAL_KHR) {
            ...
            std::fprintf(stderr, "m7_lod: vkQueuePresentKHR failed\n");
            break;
        }
